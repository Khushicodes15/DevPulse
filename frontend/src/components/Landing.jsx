...
function TypewriterLine({ text, delay, startDelay }) {
  ...
  return <div style={{ ...mono, fontSize: '13px', color: '#555560', lineHeight: '1.8' }}>{displayed}<span style={{ animation: 'fadeIn 400ms ease infinite alternate', opacity: displayed.length < text.length && started ? 1 : 0 }}>▌</span></div>
}
...
export default function Landing() {
  ...
  const connectGmail = () => {
    ...
    window.location.href = `${API}/auth/gmail/auth`
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Left column 60% */}
        <div style={{ flex: '0 0 60%', paddingRight: '60px' }}>
          <div style={{
            ...heading,
            fontSize: '48px',
            fontWeight: '800',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <button
              ...
              style={{
                ...
                border: githubConnected
                  ? 'none'
                  : `1px solid ${hoverGithub ? T.text : T.accent}`,
            ...
            >
              {githubConnected
                ? `GITHUB — ${githubHandle.toUpperCase()} ✓`
                : 'CONNECT GITHUB'}
            </button>
            ...
            <button
              ...
              style={{
                ...
                border: gmailConnected
                  ? 'none'
                  : `1px solid ${githubConnected
                    ? (hoverGmail ? T.text : T.accent)
                    : T.border}`,
                background: gmailConnected
                  ? 'transparent'
                  : (githubConnected && hoverGmail ? T.text : 'transparent'),
                color: gmailConnected
                  ? T.accent
                  : (githubConnected ? (hoverGmail ? T.bg : T.accent) : T.muted),
                ...mono,
                fontSize: '12px',
                letterSpacing: '0.1em',
                textAlign: 'left',
